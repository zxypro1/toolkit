  ...
  test('解析命令都存在', async () => {
    ...
    await Promise.all(
      runConfigs.map(async (runConfig: any) => {
        ...
        const { argsCommand, argsData, help, hangRun, parallel, singleton } =
    ...
    await Promise.all(
      runConfigs.map(async (runConfig: any) => {
        ...
        if (componentName === 'fc') {
          ...
          expect((help as IHelp).description).toBe(
            commands.deploy.subCommands.service.help.description,
          );
        } else if (componentName === 'nas') {
          await expect(async () => {
            await componentManager.parseCommands();
